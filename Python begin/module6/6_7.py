...
while current_hour <= WORKING_HOURS:
    print(f'{current_hour} час')
    all_tasks += int(input('Сколько задач решит Максим?'))
    current_call = int(input('Звонит жена. Взять трубку? (1-да, 0-нет)')) 
    if current_call > call_wife:
        call_wife = current_call
    current_hour +=1
